...
const scrapper = async (job) => {
    ...
    const { pageNo, list } = await amazonScrapper(job.url, category.nId, subCategory.nId);
    // count = + list.length;
            ...
            try {
                ...
                for (let i = 1; i <= pageNo; i++) { // insert loop
                    if (i >= (from / prodsPerPage)) {
                        const loopedData = await amazonScrapper(job.url, category.nId, subCategory.nId, i);
